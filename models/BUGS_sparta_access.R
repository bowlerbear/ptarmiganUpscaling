cat("
  ...
  for(j in 1:nvisit) {
    ...
    #detection model:
    logit(p[j]) <-  alpha.p[year[j]] + dtype.p*L[j] + access[j]*beta.access
    #depends on year and list length, and accessibility
    } 
  ...
  # State model priors
    ...
    beta.access ~ dnorm(0,0.01)
    
  }
    ",fill=TRUE,file="BUGS_sparta_access.txt")