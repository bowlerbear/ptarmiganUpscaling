...
cat("
    ...
    for(i in 1:N){
      ...
      #random.gs.year[detectionYear[i]] + random.gs.line[detectionLine[i]] + 
    ...
    for(t in 1:n.Years){
      for(j in 1:n.Lines){
        ...
        #random.gs.year[t] + random.gs.line[j] + random.gs.site2[site2[j]]+
    ...
    ",fill=TRUE,file="linetransectModel.txt")
