...
folds <- readRDS("data/folds_distanceModels.rds")
siteInfo$fold <- folds$fold[match(listlengthDF$grid,folds$grid)]

#select fold of this task
fold.id = as.integer(Sys.getenv("SGE_TASK_ID", "1"))

#split into test and train
siteInfo_test <- subset(sitInfo,fold == fold.id)
siteInfo_train<- subset(siteInfo,fold != fold.id)
tlDF_train <- subset(tlDF,grid %in% siteInfo_train)
...
allDetections <- subset(allDataObs,grid %in% siteInfo$grid)
...
bugs.data$occDM <- model.matrix(~ as.numeric(scale(siteInfo_train$bio1)) +
                                  as.numeric(scale(siteInfo_train$Open)) +
                                  as.numeric(scale(siteInfo_train$tree_line_position)) +
                                  as.numeric(scale(siteInfo_train$tree_line_position^2)))[,-1]
bugs.data$n.covs <- ncol(bugs.data$occDM)

bugs.data$occDM_test <- model.matrix(~ as.numeric(scale(siteInfo_test$bio1)) +
                                  as.numeric(scale(siteInfo_test$Open)) +
                                  as.numeric(scale(siteInfo_test$tree_line_position)) +
                                  as.numeric(scale(siteInfo_test$tree_line_position^2)))[,-1]
