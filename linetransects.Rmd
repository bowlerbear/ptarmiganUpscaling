...
#sums by hand
sum(as.numeric(bugs.data$NuIndivs),na.rm=T)#67946
sum(as.numeric(bugs.data$TransectLength),na.rm=T)#24349188
sum(as.numeric(bugs.data$NuIndivs),na.rm=T)/(sum(as.numeric(bugs.data$TransectLength),na.rm=T)/1000)
#2.790483

#mean by hand
median(as.numeric(bugs.data$NuIndivs)/(as.numeric(bugs.data$TransectLength)/1000 *0.11),na.rm=T)
#16.58925
...
#1     122 Oppland              Statskog         Kongsvoll      285    Gåvålia 67334 2970.6538
#2     122 Oppland              Statskog         Kongsvoll      285    Gåvålia 67633  709.3467
...
mapping<-unique(rp[!is.na(rp$LinjeID),c("layer","LinjeID")])
siteInfo<-merge(siteInfo,mapping,by=c("LinjeID"))

siteInfo<-unique(siteInfo[,c("layer","Rapporteringsniva","Fylkesnavn")])
names(siteInfo)[which(names(siteInfo)=="layer")]<-"grid"
#some grid are in multiple rapporteringings niva
siteInfo<-subset(siteInfo,!duplicated(grid))#need to fix
...
allDetections<-subset(allDetections,!is.na(gridIndex))
...
```{r}
#sums by hand
sum(as.numeric(bugs.data$NuIndivs),na.rm=T)
sum(as.numeric(bugs.data$TransectLength),na.rm=T)
