...
out1 <- jags(bugs.data, inits=NULL, params, "linetransectModel.txt", n.thin=nt,
               n.chains=3, n.burnin=200,n.iter=1000,parallel=T)
...
row.names(NAs)[!row.names(NAs)%in%row.names(groupInfo)]
row.names(groupInfo)[!row.names(groupInfo)%in%row.names(NAs)]
# "101" "701" 
#In "757" there was!!!! but observations were deleted by quality check
row.names(transectLengths)[!row.names(transectLengths)%in%row.names(groupInfo)]
row.names(groupInfo)[!row.names(groupInfo)%in%row.names(transectLengths)]#702 too
removeGrids<-c("101","701","757","702")
